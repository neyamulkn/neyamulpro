...
<div class="input-container">
    <div class="input-container">
        ...
        <input name="filter_name" value="{{$get_data->filter_name}}" type="text" placeholder="Enter filter name...">
    </div>
</div>
    ...
    <label for="Category" class="select-block">
        <select name="subcategory_id[]" id="Category" multiple="multiple" style="width:100%" class="select2">
            ...
            @foreach($get_category as $category)
                <option value="{{$category->id}}" {{ ( in_array($category->id, explode(',', $get_data->subcategory_id)) ) ? 'selected' : '' }} >{{$category->subcategory_name}}</option>
            @endforeach
        </select>
        <!-- SVG ARROW -->
        <svg class="svg-arrow">
