...
@section('title', 'workplace category')
			      ...
			      <?php $serial = 0; ?>
			    	@foreach($get_category as $show_category)
				      <tr id="item{{$show_category->id}}">
				        ...
				        <td>{{$show_category->sorting}}</td>
				        <td>{{$show_category->status}}</td>
				        <td>
				        	<button type="button" onclick="edit('{{$show_category->id}}')"  data-toggle="modal" data-target="#edit" class="btn btn-info">Edit</button> |
							<button type="button" onclick="deleteItem('{{ $show_category->id }}' )" class="btn btn-danger">Delete</button>
						</td>
				      </tr>
			      	@endforeach
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body form-box-item">
     		<form action="{{url('dashboard/workplace/category')}}" data-parsley-validate method="post" id="profile_info">
				 {{ csrf_field() }}
...
<script type="text/javascript">
	  ...
	  function edit(id){
            var  link = '<?php echo URL::to("/dashboard/workplace/category/edit/");?>/'+id;
            $.ajax({
            url:link,
            method:"get",
    ...
    if (confirm("Are you sure delete it.?")) {
            ...
            var  link = '<?php echo URL::to("dashboard/workplace/category/delete");?>/'+id;
            $.ajax({
            url:link,
            method:"get",
