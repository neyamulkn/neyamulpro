...
<div class="dashboard-content">
    <div class="order12lt">
      ...
      <div class="order122">
        ...
        <h1 class="ordertitle"><span class="sl-icon icon-star"></span> Order #{{$get_order_details->jobOrderId}} <a class="gig-view33" href="{{url('workplace/'.$get_order_details->job_title_slug)}}">view job</a></h1>
        <br>
    ...
    <div class="order-gig-detail">
      <div class="cart" style="width: initial;">
          ...
          <!-- /CART HEADER -->
          <form action="{{route('job_feadback', $get_order_details->jobOrderId)}}" method="post">
          
            {{csrf_field()}}
            @if(Auth::user()->role_id == env('FRELANCER'))
            <div class="cart-item">
                <div class="rating_text">
                  <h4>Communication with buyer</h4>
                  <p>How response was the buyer during the process</p>
                </div>

                <div class="rating_star">
                   <fieldset class="rating">
                    @for($i=5; $i>=1; $i--)
                    <input type="radio" id="ratting{{$i}}"  @if($i==$get_order_details->ratting_buyer) checked @endif name="ratting" value="{{$i}}" />
                    <label class = "full" @if($i<=$get_order_details->ratting_buyer) style="background-color: #FEC42D !important;"  @endif title="Star {{$i}}"  for="ratting{{$i}}"></label>
            ...
            <div style="padding: 15px;overflow: hidden;">
              ...
              <textarea name="feedback_msg" required="" minlength="6" maxlength="5000" style="width: 100% !important;margin:0px;border:1px solid #ccc; " id="textarea" placeholder="Enter feadback ...">{{ $get_order_details->feedback_buyer }}</textarea>
            </div>
           @endif
           @if(Auth::user()->role_id == env('BUYER'))
            <div class="cart-item">
                ...
                <div class="rating_star">
                  <fieldset class="rating">
                    @for($i=5; $i>=1; $i--)
                    <input type="radio" id="ratting{{$i}}" @if($i==$get_order_details->ratting_freelancer) checked @endif name="ratting" value="{{$i}}" />
                    <label class = "full" title="Star {{$i}}" @if($i<=$get_order_details->ratting_freelancer) style="background-color: #FEC42D !important;"  @endif  for="ratting{{$i}}"></label>
            ...
            <div style="padding: 15px;overflow: hidden;">
              <h4>What was like working with this seller</h4>
              <textarea name="feedback_msg" required="" minlength="6" maxlength="5000" style="width: 100% !important;margin:0px;border:1px solid #ccc; " id="textarea" placeholder="Enter feadback ...">{{ $get_order_details->feedback_freelancer }}</textarea>
            </div>
            @endif
            
            <div style=" width: 100%; padding-top: 5px;">

              <div style="float: left;"><a href="{{route('job_order_details', $get_order_details->jobOrderId)}}">Back to Order</a></div>
              <div style="float: right;">
              <a href="{{route('job_order_details', $get_order_details->jobOrderId)}}" style="float: left; margin: 8px ;">Skip</a>  
      ...
      <div class="form-box-item not-padded">
          ...
          <div class="vprogress">
              ...
              <div class="circle">
                ...
                <a href="{{route('job_feadback', $get_order_details->jobOrderId) }}" class="title progress_right">Give Feadback</a>
              </div>
              @endif
          </div>
        </div>
