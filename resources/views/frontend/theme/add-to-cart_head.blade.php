	...
	<?php if(count($get_cart_info)>0){ ?>
		...
		@foreach($get_cart_info as $get_cart)
			...
			<li class="dropdown-item">
				<a href="{{route('theme_detail', [$get_theme->theme_url])}}" class="link-to"></a>
				<!-- SVG PLUS -->
				<svg class="svg-plus">
		...
		@endforeach
			<li class="dropdown-item">
				<a href="{{route('theme.view_cart')}}" class="button tertiary half">Go to Cart</a>
				<a href="#" class="button secondary half">Go to Checkout</a>
			</li>
			<!-- /DROPDOWN ITEM -->
		</ul>
	<?php } ?>
