    ...
    public function userProtfolio(Request $request){
        ...
        elseif($request->platform == 'marketplace'){
            $data['get_gigs'] = DB::table('gig_basics')
                            ...
                            ->where('users.username', $username)
                            ->select('gig_basics.*', 'gig_prices.basic_p', 'gig_images.image_path', 'users.username', 'users.name', 'userinfos.user_image')->paginate(3);
            ...
            $data['get_theme_info'] = DB::table('themes')
                ...
                ->select('themes.*', 'theme_category.category_name', 'users.name', 'users.id', 'userinfos.user_image', 'users.username', 'theme_subcategory.subcategory_name')
                ->where('users.username', $username)
                ->groupBy('themes.theme_id')->paginate(2);
