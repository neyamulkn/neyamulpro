...
class GigOrderController extends Controller
{
    ...
    public function order_checkout(Request $request)
    {
        ...
        Session::put('package_name', $request->package);
            ...
            $get_gigs = DB::table('gig_basics')
                ...
                ->select('gig_basics.gig_id', 'gig_basics.gig_title', 'gig_basics.gig_url', 'gig_prices.*', 'gig_images.image_path', 'gig_basics.gig_user_id')
               
                ->where('gig_basics.gig_id', '=',  $request->gig_id)->first();

                Session::put('buy_gig_id', $get_gigs->gig_id);
                Session::put('gig_seller_id', $get_gigs->gig_user_id);
    ...
    public function wishList(Request $request)
    {
        ...
        if(!$get_cart_info){
            ...
            if($insert_id){
                ...
                return redirect::route('gig_checkout', $insert_id);
            }else{
                return back();
            }
        }else{
            ...
            return redirect::route('gig_checkout', $get_cart_info->cart_id);
        }
    }
    ...
    {   
        ...
        if($request->has('quantity')){
            ...
            $subtotal = session::get('subtotal');
            $total = $quantity*$subtotal+2;
            Session::put('item_quantity', $quantity);
    ...
    public function payment_success()
    {
        ...
        if(Session::has('refferel_user_name')){
           ...
           $order_id = $user_id . strtoupper(substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), -8));
            ...
            $data = [
                'order_id' => $order_id,
                'gig_id' => session::get('buy_gig_id'),
                'ref_user' => $ref_user,
                'package_name' => session::get('package_name'),
                'quantity' => $quantity,
                'subtotal' => $subtotal,
                'total' => $total,
                'seller_id' =>  session::get('gig_seller_id'),
            ...
            if($insert){
               // forget payment id and status
                ...
                return redirect::route('order_requirement', $order_id)->with('msg', 'Thank you for your order');
            }else{
    ...
    public function manage_order(){
        ...
        if($user_role == env('FRELANCER')){
            $get_order =  $get_order->where('gig_orders.seller_id' , '=', $user_id)->get();
            return view('backend.seller-orders')->with(compact('get_order'));
        }else{
            $get_order =  $get_order->where('gig_orders.buyer_id' , '=', $user_id)->get();
    ...
    public function order_completed(Request $request, $order_id){
        ...
        if($get_order_info){
            ...
            if($success){ 
                ...
                User::find($get_order_info->seller_id)->increment('wallet', $earning);
                ...
                return redirect::route('gig_feadback', $order_id);
                exit();
            }
        }
    ...
    public function feadback($order_id)
    {
        ...
        $get_order_details = DB::table('gig_orders')
            ->join('gig_basics', 'gig_orders.gig_id', 'gig_basics.gig_id')
            ->leftJoin('feedback', 'gig_orders.order_id', 'feedback.order_id');
            ...
            $get_order_details =  $get_order_details->join('userinfos', 'users.id', '=', 'userinfos.user_id')
                ...
                ->select('gig_orders.order_id as gigOrderId', 'gig_orders.subtotal', 'gig_orders.status', 'feedback.*', 'users.username', 'userinfos.user_image', 'gig_basics.gig_title', 'gig_basics.gig_url', 'gig_basics.gig_payment_type')->first();
   
            if($get_order_details){
                return view('backend.feadback')->with(compact('get_order_details'));
    ...
    public function insert_feadback(Request $request, $order_id){
        $user_id = Auth::user()->id;
        $get_order = DB::table('gig_orders')->where('order_id', $order_id)
        ...
        if($get_order){
            ...
            if(Auth::user()->role_id == env('FRELANCER')){
               $data = [
                ...
                'buyer_ratting' => $request->buyer_ratting,
                'buyer_feadback_msg' => $request->buyer_feadback_msg,
                ];
            }
            ...
            GigFeedback::updateOrCreate(['order_id' => $order_id, 'gig_id' => $get_order->gig_id], $data);

            Toastr::success('Thanks for give your feadback.');
            return redirect::route('gig_order_details', $order_id);
